...
export default function Signup() {
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    axios
      .get(import.meta.env.VITE_BASE_URL + `/auth/validate?email=${formdata.email}`)
      .then(({ data }) => {
        if (data.exist) {
    ...
    <>
      ...
      <form className="space-y-4">
        <label className="label">
          이메일
          <input className="input" type="text" name="email" onChange={handleChange} placeholder="이메일을 입력하세요" />
          <button
        ...
        <button className="wideButton" onClick={handleSubmit} disabled={!emailValidation}>
          제출하기
        </button>
      </form>
    </>
